Shader "Unlit/WaterfallInteractive"
{
    Properties
    {
        [Space]
    ...
    SubShader
    {
		...
		float3 flowDir= -(worldNormal * 2.0f) - 1.0f;
            ...
            float3 SideFoamNoiseZ = tex2D(_SideNoiseTex,
                                          float2(IN.worldPos.z * 10, IN.worldPos.y + vertFlow) * _NoiseScale);
            float3 SideFoamNoiseX = tex2D(_SideNoiseTex,
                                          float2(IN.worldPos.x * 10, IN.worldPos.y + vertFlow) * _NoiseScale);
            ...
            // depth
            half4 foamLine = 1 - saturate(_Foam * float4(noisetexture, 1) * (depth - IN.screenPos.w));
		...
		half rim = 1.0 - saturate(dot(normalize(IN.viewDir) , o.Normal));
            ...
            float3 foam = (smoothstep(_TopSpread, _TopSpread + _Softness, worldNormalDotNoise) * smoothstep(
                worldNormalDotNoise, worldNormalDotNoise + _Softness, _TopSpread + _EdgeWidth));
